...
### 使每个 lpName 都不相等的原因
因为若是每个 lpName 都相等的话，你就算调用再多次 CreateEventW，到最后换页池中也只存在着一块 lpName（因为是字符串，只要有一份就够了），也就是说到最后 PPPagedLookasideList 248字节链上只会存在一块空闲换页池，那要是这个换页池在我们的漏洞触发前被别人申请了呢？所以填满PPPagedLookasideList 248字节链才是最安全的。
